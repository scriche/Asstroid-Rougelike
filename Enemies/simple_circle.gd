...
@export var rotation_speed: float = 1.0 # How fast the "firing point" spins
...
func _physics_process(delta: float) -> void:
	...
	# 3. ROTATE TOWARDS TARGET (Your math, modularized)
	...
	if fire_timer > 3: # Fire every half second
		#fire_circle_pattern()
		fire_towards_player()
		fire_timer = 0.0
...
func _spawn_bullet(angle: float):
	...
	b.scale = Vector2(4, 4) # Adjust bullet size if needed
